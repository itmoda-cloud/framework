...
use ITModa\Framework\Event\PluginInstalled;
use ITModa\Framework\Event\PluginUninstalled;
...
class PluginCommand extends HyperfCommand
{
    ...
    public function configure(): void
    {
        ...
        $this->setDescription('plugin of itmoda install');
    }
...
//                $this->uninstall();
    ...
    {
        $this->line(sprintf('ITModa插件[%s:%s]开始安装', $this->package, $this->version), 'info');
        ## 下载文件
    ...
    {
        $this->line(sprintf('ITModa插件[%s:%s]开始卸载', $this->package, $this->version), 'info');
        ## vendor链接删除
    ...
    protected function archiveDownload(bool $isCover = false): bool
    {
        if ($isCover === false && file_exists($this->archiveFile)) {
            $this->line('ITModa插件[归档]已存在', 'info');
            return true;
        }
        ...
        $res = false;
        if ($res) {
            $this->line('ITModa插件[归档]下载完成', 'info');
        }
    ...
    protected function zipExtract(): bool
    {
        if (file_exists($this->pkgInstallDir . '/composer.json')) {
            $this->line('ITModa插件[归档文件]已经解压', 'info');
            return true;
        }

        $zip = new \ZipArchive();
        if ($zip->open($this->archiveFile) !== true) {
            $this->line($this->archiveFile . 'ITModa插件[归档文件]打开失败', 'error');
            return false;
        }

        $extractRes = $zip->extractTo($this->pkgInstallDir);
        if (! $extractRes) {
            $this->line($this->archiveFile . 'ITModa插件[归档解压]失败', 'error');
            $zip->close();
            return false;
        }

        $zip->close();
        $this->line('ITModa插件[归档解压]完成', 'info');
        return true;
    }
    ...
    protected function composerInstall(): bool
    {
        ...
        if ($shRes['signal'] === false || $shRes['code'] !== 0) {
            $falseMsg = 'ITModa插件[composer链接]错误';
            isset($shRes['output']) && $falseMsg .= ':' . $shRes['output'];
            $this->line($shRes, 'error');
            return false;
        }

        $this->line('ITModa插件[composer链接]完成', 'info');
        return true;
    }
    ...
    protected function composerUninstall(): bool
    {
        ...
        $shRes = System::exec($repoSh);
        if ($shRes['signal'] === false || $shRes['code'] !== 0) {
            $falseMsg = 'ITModa插件[composer.unlink]错误';
            isset($shRes['output']) && $falseMsg .= ':' . $shRes['output'];
            $this->line($shRes, 'error');
            return false;
        }

        $this->line('ITModa插件[composer.unlink]完成', 'info');
        return true;
    }
    ...
    {
//        $this->line('ITModa插件[表数据更新]完成', 'info');
        return true;
    }
    ...
    {
//        $this->line('ITModa插件[表数据删除]完成', 'info');
        return true;
    }
