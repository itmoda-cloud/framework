...
class StartServer extends Command
{
    ...
    {
        $this
            ->setDescription('Start itmoda servers.')
            ->addOption('daemonize', 'd', InputOption::VALUE_OPTIONAL, 'itmoda server daemonize', false)
    ...
    private function startServer()
    {
        ...
        if ($this->daemonize) {
            ...
            $this->io->success('itmoda server start success.');
        }
    ...
    private function stopServer()
    {
        ...
        if ($pid && Process::kill($pid, SIG_DFL)) {
            if (!Process::kill($pid, SIGTERM)) {
                $this->io->error('old itmoda server stop error.');
                die();
            }
